...
updateSpatial <- function(compilationType, samplesites, mapPath){
  if(file.exists(file.path(mapPath, paste0("spatiallookup_", compilationType, ".rds")))){
    ...
    if(dir(mapPath, pattern = "BEC_map") %in% previousMapVer$mapFile){ # bec map does not change
      samples_skip_bec <- samples_skip[BEC_SOURCE_prev == 1,
                                       .(SITE_IDENTIFIER,
                                         BEC = BEC_ZONE_prev,
                                         BEC_SBZ = BEC_SBZ_prev,
                                         BEC_VAR = BEC_VAR_prev,
                                         BEC_SOURCE = BEC_SOURCE_prev)]
      if(nrow(samples_proc) > 0){
  ...
  } else { # no lookup table is found
    samplesites_Loc <- unique(samplesites[,.(SITE_IDENTIFIER,
                                             IP_UTM, IP_NRTH, IP_EAST)],
                              by = "SITE_IDENTIFIER")
    samples_proc <- samplesites_Loc[!is.na(IP_UTM) & !is.na(IP_NRTH) & !is.na(IP_EAST),
                                    .(SITE_IDENTIFIER,
                                      IP_UTM, IP_NRTH, IP_EAST)]
    ## for albers and latlong
    if(nrow(samples_proc) > 0){
      ...
      names(samplesites_loc_bec) <- c("SITE_IDENTIFIER", "BEC", "BEC_SBZ", "BEC_VAR")
      samplesites_loc_bec[!is.na(BEC), BEC_SOURCE := 1] # 1 means from map
      samplesites <- merge(samplesites, samplesites_loc_bec,
                           by = "SITE_IDENTIFIER", all.x = TRUE)
      rm(samplesites_loc_bec)
      # for TSA
      tsamap <- readRDS(dir(mapPath, "TSA_", full.names = TRUE))
      samplesites_loc_tsa <- FAIBBase::getSpatial(pointID = samples_proc$SITE_IDENTIFIER,
                                                  zone = samples_proc$IP_UTM,
      ...
      samplesites[,':='(BC_ALBERS_X = as.numeric(NA),
                        BC_ALBERS_Y = as.numeric(NA),
                        Longitude = as.numeric(NA),
                        Latitude = as.numeric(NA),
                        BEC = as.character(NA),
                        BEC_SBZ = as.character(NA),
                        BEC_VAR = as.character(NA),
                        BEC_SOURCE = as.numeric(NA),
                        TSA = as.character(NA),
                        TSA_DESC = as.character(NA),
                        FIZ = as.character(NA),
                        TFL = as.character(NA),
                        TFL_LICENCEE = as.character(NA),
                        OWNER = as.numeric(NA),
                        SCHEDULE = as.character(NA),
                        OWNERSHIP_DESCRIPTION = as.character(NA))]
    }
  }
  return(samplesites)
}
